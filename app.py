...
def runall():
    ...
    def upload_directory_to_s3(local_directory, s3_bucket, s3_prefix):
        ...
        for root, _, files in os.walk(local_directory):
            for file in files:
                ...
                relative_path = os.path.relpath(local_path, local_directory)  # Keep relative structure
    ...
    if __name__ == "__main__":
        # S3 PDF Path (Input)
        pdf_s3_path = "IPCA/a07b389c-430a-4d0d-baf5-9bb05bda36a9/BRSR/brsr_input/IPCA.pdf"

        # Derive dynamic S3 Output Path by replacing `brsr_input` with `brsr_output`
        output_s3_dir = pdf_s3_path.replace("brsr_input", "brsr_output").rsplit("/", 1)[0] + "/"
        ...
        local_pdf_path = os.path.join(tempfile.gettempdir(), "IPCA.pdf")
        ...
        s3_client.download_file(AWS_BUCKET_NAME, pdf_s3_path, local_pdf_path)
    ...
    def connect_to_db():
        """Establishes connection to the PostgreSQL database using .env credentials."""
        try:
    ...
    def get_next_id(cursor):
        ...
        if not max_id or not isinstance(max_id, str):  # If the table is empty, start from '10000A'
            return "10000A"

        try:
            num_part = int(max_id[:-1])  # Extract numeric part (10000)
            letter_part = max_id[-1]  # Extract last character (A-Z)

            if letter_part not in string.ascii_uppercase:
                return "10000A"

            if letter_part == 'Z':  # If 'Z', increment number and reset to 'A'
                num_part += 1
                next_letter = 'A'
            else:
    ...
    def process_json_files_from_s3(s3_directory):
        ...
        try:
            ...
            response = s3_client.list_objects_v2(Bucket=AWS_BUCKET_NAME, Prefix=s3_directory)
            ...
            for s3_key in json_files:
                try:
                    # Extract the section name from the S3 path
                    path_parts = s3_key.split("/")

                    # Extract section correctly
                    ...
                    # Extract subsection (e.g., "i Details of the entity", "Principle_1")
                    if len(path_parts) > 3:
                        subsection_name = path_parts[-2]
                    else:
                        subsection_name = "Unknown Subsection"

                    # Final table name format
                    ...
                    file_obj = s3_client.get_object(Bucket=AWS_BUCKET_NAME, Key=s3_key)
                    json_data = json.load(io.BytesIO(file_obj['Body'].read()))

                    # Generate unique ID
                    unique_id = get_next_id(cur)
                    current_timestamp = datetime.now(IST).strftime('%Y-%m-%d %H:%M:%S.%f')[:-3]

                    # File link and record type
                    file_link = "s3://79e4192c-0969-4476-95ae-cf3af722bc98/IPCA/a07b389c-430a-4d0d-baf5-9bb05bda36a9/BRSR/brsr_input/IPCA.pdf"
                    record_type = "IPCA Laboratories Limited Annual Report 2023-24"
    ...
    if __name__ == "__main__":
        # S3 JSON directory path
        s3_json_directory = "IPCA/a07b389c-430a-4d0d-baf5-9bb05bda36a9/BRSR/brsr_output/"

        print(f"📂 Fetching JSON files from S3 directory: s3://{AWS_BUCKET_NAME}/{s3_json_directory}")
        
        process_json_files_from_s3(s3_json_directory)
